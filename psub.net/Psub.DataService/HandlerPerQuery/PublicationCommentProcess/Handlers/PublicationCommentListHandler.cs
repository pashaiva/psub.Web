...
namespace Psub.DataService.HandlerPerQuery.PublicationCommentProcess.Handlers
{
    public class PublicationCommentListHandler : IQueryHandler<PublicationCommentListQuery, ListPublicationComment>
    {
        ...
        public ListPublicationComment Handle(PublicationCommentListQuery catalog)
        {
            var commentList = _publicationCommentRepository.Query().Where(m => m.Publication.Id == catalog.Id && m.AnswerTo == null).Fetch(m => m.Replys).OrderByDescending(m => m.Id).ToList();
